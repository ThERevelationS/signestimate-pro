...
import { BrickStoneInventory } from "@/entities/all";
...
export default function BrickStoneInventoryPage() {
  ...
  const loadInventory = async () => {
    setIsLoading(true);
    try {
      const data = await BrickStoneInventory.list('-created_date');
      setInventory(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await BrickStoneInventory.update(editingItem.id, formData);
      } else {
        await BrickStoneInventory.create(formData);
      }
      await loadInventory();
      closeDialog();
    } catch (error) {
  ...
  const deleteItem = async (id, name) => {
    if (confirm(`Are you sure you want to delete "${name}"?`)) {
      try {
        await BrickStoneInventory.delete(id);
        await loadInventory();
      } catch (error) {
          ...
          <Card className="bg-rose-50 border-rose-200">
            <CardContent className="p-6">
              <div className="text-center">
                <p className="text-3xl font-bold text-rose-900">{inventory.length}</p>
                <p className="text-sm text-rose-700">Total Materials</p>
              </div>
            </CardContent>
          </Card>
