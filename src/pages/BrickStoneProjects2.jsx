...
import { BrickStoneProject2, User } from "@/entities/all";
...
export default function BrickStoneProjects2() {
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      const projectsData = await BrickStoneProject2.filter({ created_by: user.email }, '-created_date');
  ...
  const deleteProject = async (projectId, projectName) => {
    if (confirm(`Are you sure you want to delete "${projectName}"? This action cannot be undone.`)) {
      try {
        await BrickStoneProject2.delete(projectId);
        await loadProjects();
      } catch (error) {
  ...
  const navigateToEdit = (project) => {
    navigate(createPageUrl(`NewBrickStoneEstimate2?edit=${project.id}`));
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
              ...
              Brick & Stone 2 Projects
            </h1>
            <p className="text-slate-600">Manage your sign base estimates</p>
          </div>
          <Link to={createPageUrl("NewBrickStoneEstimate2")}>
                ...
                {filteredProjects.length === 0 ? (
                  ...
                  <div className="space-y-0">
                    {filteredProjects.map((project) => (
                      <div
                        key={project.id}
                        className={`p-6 border-b border-slate-50 last:border-b-0 hover:bg-pink-50 transition-colors cursor-pointer ${
                          selectedProject?.id === project.id ? 'bg-pink-50 border-l-4 border-pink-500' : ''
                        }`}
              ...
              <Card className="bg-white border-0 shadow-sm sticky top-8">
                ...
                <CardContent className="space-y-6 pt-6">
                  ...
                  <div className="bg-pink-50 p-4 rounded-lg border border-pink-200">
                    <h4 className="font-medium text-pink-900 mb-3">Base Specifications</h4>
                    <div className="space-y-2 text-sm text-pink-800">
                      ...
                      {selectedProject.material_name && (
                        <>
                          <div className="flex justify-between border-t border-pink-300 pt-2 mt-2">
                            <span>Material:</span>
