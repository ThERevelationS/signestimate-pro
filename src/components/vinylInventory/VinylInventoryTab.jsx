...
//
// The same component is used in all three places to keep the editor identical;
// the only differences are which items are shown and what badge/colors appear.
...
export default function VinylInventoryTab({ scope = "master" }) {
  ...
  // Optimistic patch — update locally, then persist. Reverts on error.
  const handlePatch = async (id, patch) => {
    if (!isAdmin) return;
    const before = items;
    setItems(prev => prev.map(v => v.id === id ? { ...v, ...patch } : v));
    try {
      await VinylInventory.update(id, patch);
    } catch (e) {
      console.error(e);
      setItems(before);
          ...
          <div className="space-y-2">
            {filtered.map(v => (
              <VinylRowCard
                ...
                vinyl={v}
                isAdmin={isAdmin}
                onPatch={(patch) => handlePatch(v.id, patch)}
